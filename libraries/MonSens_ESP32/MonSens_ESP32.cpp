...
#include <MonSens_ESP32.h>

/**
 * Set the SSID of the WiFi network.
 */
void MonSens_ESP32::setSsid(const char* ssid) {
  wifiSsid = ssid;
}
 ...
 */
void MonSens_ESP32::setPassword(const char* password) {
 ...
 */
void MonSens_ESP32::setPort(const uint16_t port) {
  tcpPort = port;
}
 ...
 */
void MonSens_ESP32::communicate() {
  ...
  // Wait until the client sends some data, but not more then MONSENS_ESP32_TIMEOUT ms
  uint16_t clientTimeout = 0;
  while (!client.available() && clientTimeout <= MONSENS_ESP32_TIMEOUT) {
    ++clientTimeout;
    delay(1);
  }
  if (clientTimeout > MONSENS_ESP32_TIMEOUT) {
 ...
 */
void MonSens_ESP32::write(const char output) {
  Serial.write(output);
  client.print(output);
}
